...
import { TasksAccess } from '../dataAccess/tasksAcess'
import { TasksStorage } from './attachmentUtils';
...
export async function createTaskItem(event: APIGatewayProxyEvent, createTaskRequest: CreateTaskRequest): Promise<TaskItem> {
    ...
    console.log('createTaskItem userId:' + userId + "taskId:"+userId +"bucketname:" +bucketName )
    logger.info('createTaskItem userId:' + userId + "taskId:"+userId +"bucketname:" +bucketName );
